...
const migrateLegacyColor = (color = '') => LEGACY_COLOR_MAP[color.toLowerCase()] || color
const hasChanged = <T extends Record<string, unknown>>(source: T, updates: Partial<T>) =>
  Object.entries(updates).some(([key, value]) => source[key as keyof T] !== value)
...
const createStarterComponents = (): ComponentData[] => [
  createProtocolComponent(ComponentType.TEXT, {
    name: '主标题',
    style: { top: 72, left: 72, width: 420, height: 80, fontSize: 36, fontWeight: 700, color: '#3f4746' },
    props: { content: '2026 春季增长活动，限时开启' }
  }),
  createProtocolComponent(ComponentType.TEXT, {
    name: '卖点描述',
    style: { top: 162, left: 72, width: 420, height: 72, fontSize: 16, fontWeight: 400, color: '#6b716d' },
    props: { content: '低代码搭建营销落地页，支持拖拽布局、图层管理、属性配置、实时预览与页面 JSON 导入导出。' }
  }),
  createProtocolComponent(ComponentType.BUTTON, {
    name: '主 CTA',
    style: { top: 258, left: 72, width: 180, height: 46, backgroundColor: '#6f8583', borderColor: '#6f8583' },
    props: { content: '立即报名' },
    events: [{ type: 'click', config: { action: 'url', url: 'https://example.com', newTab: true } }]
  }),
  createProtocolComponent(ComponentType.INPUT, {
    name: '手机号输入',
    style: { top: 258, left: 270, width: 220, height: 46, backgroundColor: '#faf8f3', borderColor: '#d5cec2' },
    props: { placeholder: '请输入手机号', inputType: 'tel' }
  }),
  createProtocolComponent(ComponentType.IMAGE, {
    name: '活动主视觉',
    style: { top: 348, left: 72, width: 420, height: 240, backgroundColor: '#e8e1d7', borderRadius: 18 },
    props: { src: '', alt: '活动主视觉', objectFit: 'cover' }
  }),
  createProtocolComponent(ComponentType.FORM, {
    name: '报名表单',
    style: { top: 96, left: 600, width: 360, height: 420, backgroundColor: '#faf8f3', borderColor: '#d5cec2' },
...
const normalizeComponentLayout = (component: ComponentData, page: PageData): ComponentData => {
  ...
  next.style.color = migrateLegacyColor(next.style.color)
  next.style.backgroundColor = migrateLegacyColor(next.style.backgroundColor)
  next.style.borderColor = migrateLegacyColor(next.style.borderColor)
...
const normalizePageData = (page: PageData): PageData => {
  ...
  nextPage.style.backgroundColor = migrateLegacyColor(nextPage.style.backgroundColor) || '#f4f1ea'
...
export const useEditorStore = defineStore('editor', () => {
  ...
  const updatePageMeta = (metaUpdates: Partial<PageMeta>) => {
    if (!currentPage.value) return
    if (!hasChanged(currentPage.value.meta as unknown as Record<string, unknown>, metaUpdates as Record<string, unknown>)) return

    const prevMeta = clone(currentPage.value.meta)
    const nextMeta = clone({
    ...
    useHistoryStore().executeCommand({
      ...
      execute: () => {
        if (!currentPage.value) return
        currentPage.value.meta = clone(nextMeta)
      },
      undo: () => {
        if (!currentPage.value) return
        currentPage.value.meta = clone(prevMeta)
      }
    })
  }
  ...
  const updatePageStyle = (styleUpdates: Partial<PageData['style']>) => {
    if (!currentPage.value) return
    if (!hasChanged(currentPage.value.style as unknown as Record<string, unknown>, styleUpdates as Record<string, unknown>)) return

    const prevStyle = clone(currentPage.value.style)
    const nextStyle = clone({
      ...currentPage.value.style,
      ...styleUpdates
    })
    ...
    useHistoryStore().executeCommand({
      ...
      execute: () => {
        if (!currentPage.value) return
        currentPage.value.style = clone(nextStyle)
        touchPageMeta()
      },
      undo: () => {
        if (!currentPage.value) return
        currentPage.value.style = clone(prevStyle)
        touchPageMeta()
      }
    })
  }
